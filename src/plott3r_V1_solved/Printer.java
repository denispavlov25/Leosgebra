...
public class Printer {
    ...
    int offsetX = 85; //eine Art Konstante (die ungefähre Mitte des A4-Blattes) in mm
    int offsetY = 125;
    ...
    private void zeicheKoordinatenSystem(Koordinatensystem koordinatensystem) throws InterruptedException {
        ...
        roboter.moveToPosition(new Position2D(0 + offsetX, - koordinatensystem.getWert() + offsetY),3000);
        roboter.getZAchse().deaktiviere();
        /*hier wird Schritt für Schritt vom Ursprung, wo der Stift ist, das KS gezeichnet;
        * man muss aber beachten, dass wir nachdem wir das Papier vorbereitet haben, den Stift auf die andere Seite bringen müssen, damit die Werde des Rechners stimmen können*/
    ...
    public void zeichneFunktion(Function <Double, Double> f) throws InterruptedException {
        ...
        roboter.getZAchse().deaktiviere();
        roboter.moveToPosition(new Position2D(positionX + offsetX, (int)(f.apply((double)positionX) + offsetY)),3000);
        roboter.getZAchse().aktiviere();
